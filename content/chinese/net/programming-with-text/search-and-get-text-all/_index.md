...
---
本教程讲解如何使用 Aspose.PDF for .NET 搜索并获取 PDF 文档所有页面的文本。提供的 C# 源代码逐步演示了该过程。
...
### 使用 Aspose.PDF for .NET 搜索并获取文本的示例源代码 
```csharp
//文档目录的路径。
string dataDir = "YOUR DOCUMENT DIRECTORY";
//打开文档
...
恭喜！您已成功学习了如何使用 Aspose.PDF for .NET 从 PDF 文档的所有页面中搜索和获取文本。本教程提供了分步指南，从加载文档到访问提取的文本片段。您现在可以将此代码合并到您自己的 C# 项目中，以分析和处理 PDF 文件中的文本内容。

### 常见问题解答

#### 问：“搜索并获取全部文本”教程的目的是什么？

答：“搜索并获取全部文本”教程演示了如何利用 .NET 的 Aspose.PDF 库从 PDF 文档的所有页面中搜索和提取文本。该教程提供了分步说明以及用于执行文本搜索和检索的示例 C# 代码。
...
答：您可以使用`TextFragmentAbsorber`类用于在 PDF 文档中查找特定搜索短语的实例。通过创建此类的实例并指定目标文本，您可以捕获该文本的所有出现情况。

#### 问：我可以搜索 PDF 文档所有页面上的文本吗？

答：是的，本教程演示了如何在 PDF 文档的所有页面中搜索文本。`pdfDocument.Pages.Accept(textFragmentAbsorber)`方法用于接受所有页面的吸收器，允许您在每一页上搜索所需的文本。

#### 问：如何访问提取的文本片段？

答：搜索文本后，您可以使用`TextFragments`的财产`TextFragmentAbsorber`对象。此属性提供对以下对象的集合的访问：`TextFragment`包含提取的文本和相关信息的对象。
...
答：当然可以。获得提取的文本片段后，您可以修改循环内的代码以对每个片段执行自定义操作。这可能包括保存提取的文本、分析文本模式或应用格式更改。