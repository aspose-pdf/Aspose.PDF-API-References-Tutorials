...
实例化`HtmlFragment`对象并提供所需的 HTML 内容。在提供的代码中，HTML 内容被分配给变量`titel`。您可以根据需要修改HTML内容。
...
## 步骤 7：设置边距信息
如果需要，请调整 HTML 片段的底部和顶部边距。在提供的代码中，底部边距设置为 10，顶部边距设置为 200。
...
## 步骤 10：显示成功消息
显示成功消息以及保存 PDF 文件的路径。

```csharp
Console.WriteLine("\nHTML using DOM added successfully.\nFile saved at " + dataDir);
```

### 使用 Aspose.PDF for .NET 添加 HTML 使用 DOM 的示例源代码 
```csharp
//文档目录的路径。
...
## 结论
您已成功使用 Aspose.PDF for .NET 中的 DOM 添加 HTML 内容。现在可以在指定的输出文件路径中找到生成的 PDF 文件。

### 常见问题解答

#### 问：本教程的目标是什么？

答：本教程旨在提供分步指南，介绍如何使用 Aspose.PDF for .NET 中的文档对象模型 (DOM) 将 HTML 内容添加到 PDF 文档。它包含 C# 源代码片段，以帮助您理解和实施该过程。
...
答：在步骤 4 中，实例化一个新的`Document`对象，添加以下代码行：
...
答：在第 6 步中，你将创建一个`HtmlFragment`对象并为其分配所需的 HTML 内容。HTML 内容被分配给变量`titel`：
...
答：是的，在第 7 步中，您可以根据需要调整 HTML 片段的底部和顶部边距：
...
答：添加 HTML 内容并调整边距后，使用`Save`方法`Document`保存PDF文档的对象：
...
答：当然，在第 10 步中，会显示成功消息以及 PDF 文件的保存路径：
...
答：通过本教程，您已成功学会了如何利用 Aspose.PDF for .NET 中的文档对象模型 (DOM) 将 HTML 内容添加到 PDF 文档。这些知识将帮助您增强 PDF 生成能力。