---
title: Image et numéro de page dans la section d'en-tête et de pied de page en ligne
linktitle: Image et numéro de page dans la section d'en-tête et de pied de page en ligne
...
Une fois l'en-tête avec l'image et le texte en ligne ajouté, nous pouvons enregistrer le document PDF modifié. Voici comment:
...
### Exemple de code source pour l'image et le numéro de page dans la section de pied de page d'en-tête en ligne à l'aide d'Aspose.PDF pour .NET 
```csharp
...
// Créez un objet image dans la section
Aspose.Pdf.Image image1 = new Aspose.Pdf.Image();
...
// Indiquez que InlineParagraph de seg1 est une image.
image1.IsInLineParagraph = true;
Aspose.Pdf.Text.TextFragment txt2 = new Aspose.Pdf.Text.TextFragment(" Pty Ltd.");
txt2.IsInLineParagraph = true;
txt2.TextState.ForegroundColor = Color.Maroon;
...
Félicitation ! Vous avez appris à ajouter une image et un numéro de page dans la section d'en-tête et de pied de page d'un document PDF à l'aide de paragraphes en ligne avec Aspose.PDF pour .NET. Vous pouvez désormais personnaliser de manière flexible l'en-tête et le pied de page de vos documents PDF.
...
#### Q : Comment puis-je spécifier la couleur du texte en ligne dans l'en-tête ?

 R : La couleur du texte en ligne est spécifiée à l'aide du`ForegroundColor` propriété du`TextState` de la`TextFragment` objet.

#### Q : Puis-je ajuster les dimensions de l'image en ligne dans l'en-tête ?

 R : Oui, vous pouvez ajuster les dimensions de l'image en ligne à l'aide du`FixWidth` et`FixHeight` propriétés du`Image` objet. Cela vous permet de contrôler la largeur et la hauteur de l'image dans l'en-tête.

#### Q : Puis-je inclure des éléments en ligne supplémentaires, tels que des hyperliens ou différents styles de police, dans l'en-tête ?

 R : Oui, vous pouvez inclure des éléments en ligne supplémentaires dans l'en-tête en créant davantage d'éléments en ligne.`TextFragment` ou`Image` objets ayant les propriétés souhaitées. Cela vous permet de personnaliser davantage l'en-tête, y compris des hyperliens, différents styles de police ou d'autres éléments visuels.

#### Q : Comment puis-je m'assurer que l'image et le texte en ligne restent correctement alignés et formatés sur différents appareils et visualiseurs ?
...
#### Q : Comment puis-je appliquer des paragraphes en ligne à des pages spécifiques du document PDF ?
 ...
 R : Pour appliquer des paragraphes en ligne à des pages spécifiques, vous pouvez créer des`HeaderFooter` objets pour chaque page et attribuez-les à l'aide du`Header` propriété du respectif`Aspose.Pdf.Page` objets.