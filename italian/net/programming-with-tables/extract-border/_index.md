...
Assicurati di sostituire "YOUR DOCUMENT DIRECTORY" con la directory effettiva in cui si trova il tuo file PDF.

## Passaggio 3: estrazione del bordo
Estrarremo il bordo dal documento PDF ripetendo le operazioni contenute nel documento.
...
{
     // Elabora tutte le operazioni sui contenuti
     foreach(Operator op in doc.Pages[1].Contents)
     {
         // Verificare il tipo di operazione
         // ...
         // Aggiungere il codice per elaborare ogni operazione
     }
}
```

 Creiamo un`graphicsState` stack per memorizzare gli stati grafici, un'immagine bitmap per catturare il bordo estratto, a`GraphicsPath` oggetto per memorizzare i percorsi di disegno e altre variabili per tenere traccia dello stato e dei colori.
...
// ...
// Aggiungi else if blocchi per altre operazioni
```

Controlliamo il tipo di operazione utilizzando le condizioni ed eseguiamo il codice appropriato per ogni operazione.
...
using (System.Drawing.Graphics gr = System.Drawing.Graphics.FromImage(bitmap))
{
	...
	// Elabora tutti i comandi contenuti
	foreach (Operator op in doc.Pages[1].Contents)
	{
