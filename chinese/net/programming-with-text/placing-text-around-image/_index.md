...
接下来，我们创建一个`Document`对象并使用`Pages.Add()`方法。
...
我们设置表格的列宽并创建一个`MarginInfo`对象来设置边距。
...
我们创造一个`Image`object，指定图片文件路径，并设置图片的固定高宽。然后，我们将图像添加到表格单元格的段落集合中。
...
我们创建包含 HTML 格式文本的字符串变量并创建一个`HtmlFragment`目的。然后，我们将 HTML 文本添加到包含图像的表格单元格中。
...
我们创造另一个`HtmlFragment`包含其他 HTML 格式文本的对象，并将其添加到单独的表格单元格中。
...
在本教程中，您学习了如何使用 .NET 的 Aspose.PDF 库在 PDF 文档中的图像周围放置文本。按照分步指南并执行提供的 C# 代码，您可以在 PDF 文档中创建表格、添加图像并在图像周围放置文本。