...
##  Crear un nuevo`Document` object.
```csharp
...
doc.Pages.Add();
```
##  Crea una lista de`Point` arrays that represent the ink gesture for the annotation.
```csharp
IList<Point[]> inkList = new List<Point[]>();
```
##  Crear un nuevo`LineInfo` object that defines the properties of the ink gesture.
```csharp
...
```
##  Crear un nuevo`Aspose.Pdf.Point` array that represents the gesture from the `LineInfo` object.
```csharp
...
```
## Agregue el gesto a la lista de gestos de tinta.
```csharp
inkList.Add(gesture);
```
##  Crear un nuevo`InkAnnotation` object that represents the link annotation.
```csharp
InkAnnotation a1 = new InkAnnotation(doc.Pages[1], new Aspose.Pdf.Rectangle(100, 100, 300, 300), inkList);
```
## Establezca el asunto y el título de la anotación.
```csharp
a1.Subject = "Test";
a1.Title = "Title";
```
## Establezca el color de la anotación.
```csharp
a1.Color = Aspose.Pdf.Color.FromRgb(System.Drawing.Color.Green);
```
##  Crear un nuevo`Border` object that defines the properties of the annotation's border.
```csharp
...
```
## Agregue la anotación a la página.
```csharp
