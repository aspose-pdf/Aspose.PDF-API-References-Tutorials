...
交換する`"YOUR DOCUMENT DIRECTORY"`マシン上の実際のパスを入力します。これはパフォーマンスのステージを設定するようなものです。すべてが適切な場所にある必要があります。

## ステップ2: PDFドキュメントを読み込む

ディレクトリの設定が完了したので、変換する PDF ドキュメントを読み込みます。ここから魔法が始まります。次のコードを使用します。
