...
おめでとうございます! Aspose.PDF for .NET を使用して、PDF ドキュメントにインライン段落としてテキストと画像を追加する方法を学習しました。このチュートリアルでは、プロジェクトのセットアップから変更したドキュメントの保存まで、ステップ バイ ステップのガイドを提供しました。これで、このコードを独自の C# プロジェクトに組み込んで、PDF ファイル内のテキストと画像のレイアウトをカスタマイズできます。
